...
const DB = {
  ...
  async getAuthRecord() {
    const store = await tx(STORE_AUTH, 'readonly');
    return promisifyRequest(store.get('pin'));
  },

  async setAuthRecord(record) {
    const store = await tx(STORE_AUTH, 'readwrite');
    return promisifyRequest(store.put({ key: 'pin', ...record }));
  },

  async getSettings() {
    const store = await tx(STORE_AUTH, 'readonly');
    const s = await promisifyRequest(store.get('settings'));
    return s || { key: 'settings', autoLockMinutes: 5 };
  },

  async setSettings(settings) {
    const store = await tx(STORE_AUTH, 'readwrite');
    return promisifyRequest(store.put({ key: 'settings', ...settings }));
  },
};
